...
async function fetchAllRecords() {
  const apiKey = process.env.IDTEMPORAL_API_KEY?.trim();
  const eventId = Number(String(process.env.EVENT_ID || 177).trim());

  if (!apiKey) {
    throw new Error('Falta IDTEMPORAL_API_KEY en el servidor.');
  }
  if (!Number.isInteger(eventId)) {
    throw new Error(`EVENT_ID inválido: "${process.env.EVENT_ID}"`);
  }
  ...
  while (true) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`La API respondió con estado ${response.status}${body ? `: ${body.slice(0, 300)}` : ''}`);
    }
